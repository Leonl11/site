...
function GalleryCard({ item, filteredItems, onSelectImage }) {
  ...
  return (
    <motion.div
      ...
      role="button"
      tabIndex={0}
      aria-label={`Переглянути фотографію «${item.title}», категорія ${item.categoryName}`}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          onSelectImage(item, filteredItems);
        }
      }}
      onClick={() => onSelectImage(item, filteredItems)}
      className="group relative rounded-2xl overflow-hidden cursor-pointer bg-cinema-900 border border-white/5 hover:border-gold-500/40 focus-visible:border-gold-400 focus-visible:ring-2 focus-visible:ring-gold-400/70 focus-visible:outline-none transition-all duration-500 shadow-lg hover:shadow-[0_10px_30px_rgba(0,0,0,0.8)]"
    >
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-cinema-950">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-cinema-950 via-cinema-950/20 to-transparent opacity-60 group-hover:opacity-85 transition-opacity duration-300 pointer-events-none" />

        {/* Top Badge */}
        <div className="absolute top-4 left-4 z-10">
          <span className="px-3 py-1 rounded-full text-xs font-sans tracking-wider uppercase bg-cinema-950/80 backdrop-blur-md text-gold-300 border border-gold-500/20">
            {item.categoryName}
          </span>
        </div>
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`relative px-4 sm:px-5 py-2.5 min-h-[44px] rounded-full text-xs sm:text-sm font-medium tracking-wider uppercase transition-all duration-300 focus-visible:ring-2 focus-visible:ring-gold-400 focus-visible:outline-none ${
                isActive
